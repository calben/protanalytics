...
for res in residues:
  ...
  res_log = open(logs_dir + res + ".log", "w")
...
#    scores_file.write(",".join(list(all_data.iloc[:,:5].keys())) + ",Full KDE,Affinity Cluster,KMeans Cluster," + ",".join(list(all_data.iloc[:,5:].keys())) + "\n")
    parametereters_file = open(results_dir + "-".join(parameter) + "/" + res + "-parametereters.dat", "w")
    ...
    for key, frame in grouped:
      ...
      print("Number of samples : " + str(len(frame)))
      if len(frame) < 10:
      ...
      min_samples_for_core = len(frame)/10 if len(frame)/10 > 5 else 5
...
#      parametereters_file.write(str(dpgmm.weights_) + "\n")
    ...
    full_result.to_csv(results_dir + "-".join(parameter) + "/" + res + "-scores--full.csv", index = False)
